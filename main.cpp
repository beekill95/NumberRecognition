...
void my_snn_callback(snn* neuralNetwork, int epoch, snn_type currentError) {
    if (epoch < 100)
        neuralNetwork->setLearningRate(0.001);
    else if (epoch < 300)
        neuralNetwork->setLearningRate(0.0001);
    else
        neuralNetwork->setLearningRate(0.0005);
}

void my_adaptive_callback(snn* neuralNetwork, int epoch, snn_type currentError) {
    static snn_type prevError;
    static snn_type prevLearningRate;

    if (epoch == 0) {
        prevLearningRate = 0.001;
    } else {
        if (currentError > prevError)
            prevLearningRate *= 0.5;
        else
            prevLearningRate *= 1.2;
    }

    prevError = currentError;
    neuralNetwork->setLearningRate(prevLearningRate);
}
...
//    featureSetExtractor.addFeature(new BoundaryFeature(cv::Point(4, 4), 40));
    ...
    neuralNetwork.setLearningRate(0.001);
//    neuralNetwork.setCallback(my_adaptive_callback);
